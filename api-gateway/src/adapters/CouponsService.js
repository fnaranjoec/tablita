...
export default class CouponsService {
    ...
    static async fetchCouponsByCampaign({campaign_id}) {

        const body = await got.get(`${COUPON_SERVICE_URI}/coupons/byCampaign/${campaign_id}`).json();

        return body || [];
    };

    static async fetchCouponsByDoctor({doctor_id}) {

        const body = await got.get(`${COUPON_SERVICE_URI}/coupons/byDoctor/${doctor_id}`).json();
    ...
    static async fetchCouponsByCampaignMetrics({args}) {

        const body = await got.get(`${COUPON_SERVICE_URI}/coupons/byCampaignMetrics/${args.campaign_id}/${args.from}/${args.to}`).json();

        return body || [];
    };


    static async fetchCouponsByDoctorMetrics({args}) {

        const body = await got.get(`${COUPON_SERVICE_URI}/coupons/byDoctorMetrics/${args.doctor_id}/${args.from}/${args.to}`).json();
