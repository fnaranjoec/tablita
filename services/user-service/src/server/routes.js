...
const setupRoutes = app => {
    ...
    //
    // // *************************************************************************** SEND MESSAGE BROKER ***
    // app.post( `${ENDPOINT_URI}/rabbit/send/mailing` , async (req, res, next) => {
    //
    ...
    //
    //         await broker.send('mailing', await Buffer.from(JSON.stringify(req.body.args)))
    //             .then(resp=>{
    //                 // console.log("resp==>", resp);
    //                 return res.json({
    //                     success: resp,
    //                     message: "Mailing sent...",
    //                     data: ""
    //                 });
    //             })
    //             .catch(error=>{
    ...
    // *************************************************************************** SEND MESSAGE BULL/REDIS ***
    app.post( `${ENDPOINT_URI}/bull/send/mailing` , async (req, res, next) => {
        ...
        try {
            ...
            // 2. Set data message to job
            const data = {
                from: req.body.args.from,
                to: req.body.args.to,
                subject: req.body.args.subject,
                html: req.body.args.html,
                imageBase64: req.body.args.imageBase64,
                fileName: req.body.args.fileName,
                path: req.body.args.path,
                cid: req.body.args.cid,
            };
            ...
            // const job = await sendMailQueue.add(data, options);
            const job = await sendMailQueue.add(data);

            // 4. On Complete job
            // Listen to the global completion of the queue in order to return result.
            sendMailQueue.on('global:completed', (jobId, result) => {
                // Check if id is a number without any additions
                if (/^\d+$/.test(jobId) && !res.headersSent) {
                    console.log(`Mailing Job ${jobId} completed! Result: ${result}`);
                    res.json({
                        success: true,
                        message: `Mailing Job is completed with result: ${result}`,
                        data: `Mailing Job ${jobId} completed! Result: ${result}`,
                    });
                }
            });
