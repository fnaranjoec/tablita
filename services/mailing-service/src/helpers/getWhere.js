...
const getWhere = ({filter, status=null}) => {
    ...
    for(var attributename in finalWhere) {
        ...
        switch (operator) {
            case "equal":
                where[sequelize.Op.and][attributename] = { [sequelize.Op.eq]: valueOperator};
                break;
            case "not":
    ...
    if (status !== null ) where[sequelize.Op.and][status]= {[sequelize.Op.ne] : "X"}
