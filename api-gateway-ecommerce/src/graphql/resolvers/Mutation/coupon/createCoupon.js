...
const createCouponResolver = async (obj, args, context ) => {
    ...
    switch (mediaProduct.code_type_id) {
        ...
        case FIDELIZATION_CODE.parameter_text :
            ...
            consummerProductCouponFinded = await CouponsService.fetchCouponByConsumerMedia({ args: {consummer_id: args.consummer_id, media_id: media.media_id  } });
            // console.log('consummerProductCouponFinded==>', consummerProductCouponFinded);

            if (consummerProductCouponFinded.count!==0) return new Error("008|The fidelization code has already been used, it is invalid right now!");
